...
for filepath in "${skill_files[@]}"; do
    ...
    # -------------------------------------------------------------------------
    if [ "$is_single_agent" -eq 1 ]; then
        # Single-agent: skip A3 (no spawn definitions)
        a3_pass=$((a3_pass + 1))
    else
    ...
    # -------------------------------------------------------------------------
    if [ "$is_single_agent" -eq 1 ]; then
        # Single-agent: skip A4 (no shared content markers)
        a4_pass=$((a4_pass + 1))
    else
