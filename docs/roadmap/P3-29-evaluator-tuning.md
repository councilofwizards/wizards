...
Establish a few-shot calibration system for skeptic prompts: store graded examples of passing and failing outputs in `.claude/conclave/eval-examples/`, and add a post-mortem quality rating step to pipeline completion. Over time, calibration examples and quality ratings inform skeptic prompt refinement.

## Motivation

Anthropic's harness design paper found that evaluator tuning required multiple refinement cycles. Out-of-the-box Claude is a poor QA agent — it identifies issues then talks itself out of flagging them. The best practices document recommends storing calibration examples separately from SKILL.md to keep skills lean while making evaluator rubrics precise and adjustable.
