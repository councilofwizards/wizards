...
variables, and figure out the plan → build → review workflow. The learning curve
is steeper than necessary for a tool meant to simplify development.

## Proposed Solution

1. **`/setup-project` skill**: A lightweight skill (no multi-agent team needed —
   single agent) that:
...
- This is a single-agent skill — no team spawning, no Skeptic. It's a setup
  utility.
...
- Prerequisite: CI validator must be adapted to support `type: single-agent`
  SKILL.md files.
...
- A new user can go from install to first `/plan-product` invocation in under 5
  minutes.
