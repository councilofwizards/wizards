...
The current agent roles (Researcher, Architect, DBA, Backend Engineer, etc.) are fixed in the SKILL.md files. Projects
with different needs (e.g., a data pipeline project that needs a Data Engineer instead of a Frontend Engineer) must edit
...
3. **Role templates**: Provide starter templates for common roles (Data Engineer, ML Engineer, Mobile Engineer) that
   users can drop in.
