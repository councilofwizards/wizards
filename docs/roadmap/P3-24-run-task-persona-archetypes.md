...
run-task is the only skill where the persona system breaks down entirely. It dynamically composes agents from 4 generic
archetypes (Engineer, Researcher, Writer, Skeptic) with no persona file assignments. Spawned agents are generic
templates, not Conclave characters.

## Proposed Solution

1. Create 4 new persona files for run-task's generic archetypes with fictional names and the standard
   Identity/Communication Style sections
...
- Logically follows P2-09 (Persona System Activation) — spawn prompt persona injection pattern established there should
  be applied to run-task
