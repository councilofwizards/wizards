...
**Join point**: Before Assayer reviews excavation coverage (start of Phase 2 gate).
...
- Cross-referencing requires seeing all three lenses simultaneously (a feature's business rules reference its data model).
...
- The Chronicler processes one module at a time, producing one output document per module per concern, keeping individual
