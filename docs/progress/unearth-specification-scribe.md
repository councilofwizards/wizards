...
**Fork-join in Phase 2**: All three excavators (logic, schema, boundary) are explicitly spawned simultaneously. The
instructions use "simultaneously" and "do NOT wait" to make the parallel intent unambiguous.

**Chronicler write isolation**: Explicit "NEVER read source code" constraint added to both the Orchestration Flow and
the Chronicler's spawn prompt. Gap flagging to Dig Master is required before proceeding.
