...
1. **CREATED** `plugins/conclave/shared/personas/qa-agent.md` — Maren Greystone persona with role separation table,
   checkpoint triggers, output format, write safety, cross-references
2. **MODIFIED** `plugins/conclave/skills/build-implementation/SKILL.md` — QA agent spawn def + prompt, QA GATE after
   Quality Skeptic POST-IMPLEMENTATION review, qa-testing phase, --light mode note, contract injection update, critical
   rules, failure recovery
3. **MODIFIED** `plugins/conclave/skills/build-product/SKILL.md` — Same changes mirrored for build-product; also added
   qa-verdict to Artifact Detection table
