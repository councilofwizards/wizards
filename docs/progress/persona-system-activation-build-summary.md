...
Implemented the Persona System Activation feature: 33 spawn prompt edits across
11 SKILL.md files, communication protocol sign-off convention, placeholder fix
with sync/validator toolchain updates. Discovered and fixed a bash parameter
expansion bug in the sync script that corrupted SKILL.md files during the first
sync run. All 12/12 validators pass.

## Changes

### Infrastructure (Steps 1-3)

- Added sign-off convention to
  `plugins/conclave/shared/communication-protocol.md`
- Changed `product-skeptic` to `{skill-skeptic}` placeholder in protocol
- Updated `scripts/sync-shared-content.sh`: AUTH constants, fallback defaults,
  and hardened `extract_skeptic_names` against bash parameter expansion bug with
  `{braces}` in default values
- Updated `scripts/validators/skill-shared-content.sh`: added
  `{skill-skeptic}`/`{Skill Skeptic}` normalizer patterns

### Spawn Prompts (Step 4)

- 33 spawn prompts across 11 SKILL.md files now include fictional persona names
  and titles
...
- `plugins/conclave/shared/communication-protocol.md` — Sign-off convention +
  placeholder fix
