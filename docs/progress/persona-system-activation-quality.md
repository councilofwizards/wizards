...
- The sync script bash parameter expansion fix (intermediate variables for `{braces}` in defaults) is a genuine improvement — the original pattern would fail for ANY default value containing curly braces.
- The `extract_skeptic_names` hardening (broader regex, brace stripping, display-to-slug derivation) makes the sync more resilient to corrupted state.
- Skills using Lead-as-Skeptic pattern (research-market, ideate-product, manage-roadmap) retain `product-skeptic` in their protocol rows — this is pre-existing and correct behavior (their lead acts as skeptic).
