...
- **Resolution**: Lead-as-Skeptic determined the spec's comment placement (after last pipe) resolves the issue. Verified via shell tests. Final plan incorporates the fix.

## Key Decisions

1. Comment placement after last pipe delimiter (awk $5) instead of inside table cell (awk $4) — preserves sync idempotency
