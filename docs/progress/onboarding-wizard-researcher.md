...
- Required sections: Setup, Write Safety, Checkpoint Protocol, Determine Mode, Lightweight Mode, Spawn the Team,
  Orchestration Flow, Failure Recovery, Shared Principles, Communication Protocol
...
This is a CRITICAL constraint: the setup-project skill is envisioned as a single-agent utility, but the validator
demands multi-agent scaffolding sections.
...
- P1-03 Stack Generalization (COMPLETE): The stack detection logic already exists in all 3 SKILL.md Setup sections.
  setup-project should reuse the same detection approach.
- No hard dependencies on incomplete items
- Soft relationship with P2-07 Universal Shared Principles (not started) -- if shared principles are extracted to a
  shared file, setup-project would need updating

### 7. Validator Conflict (Critical Finding)

The CI validator at `scripts/validators/skill-structure.sh` requires ALL SKILL.md files to have multi-agent sections. A
single-agent skill would fail validation. Three options:
