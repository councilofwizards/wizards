...
status: "complete"
last_action:
  "Forge Auditor APPROVED; final SKILL.md written to plugins/conclave/skills/profile-competitor/SKILL.md (48KB, 809
  lines) with auditor's checkpoint-phase clarification applied to Counter-Spy WRITE SAFETY block"
updated: "2026-04-25T17:55:00Z"
---

# DRAFT: SKILL.md for `profile-competitor`

> Scribe draft (Quill Ashmark, scribe-f2a8). This is the verbatim content to be written to
> `plugins/conclave/skills/profile-competitor/SKILL.md` after Forge Auditor approval. The shared content blocks
> (universal-principles and communication-protocol) are included verbatim with markers; the sync script will overwrite
> them but the markers and seed content must be present for validation.
...
description: >
  Invoke The Black Atlas to profile a single named competitor with comprehensive, objective deep-dive analysis. Deploys
  four parallel field agents (market, product, technical, GTM) with skeptic gates at every phase, then synthesizes
  findings into a progressive-disclosure dossier (Executive Summary with positioning recommendations → General Review →
  Technical Details → Reference Sources).
argument-hint: "[--light] [--refresh] [--refresh-after Nd] [status | <CompanyName> | (empty for resume)]"
category: planning
...
## Section Checklist (Scribe self-verification)

- [x] Frontmatter (name: profile-competitor, category: planning, tags, argument-hint, description)
- [x] `# The Black Atlas — Competitor Profiling Orchestration` H1
- [x] `## Setup`
- [x] `## Write Safety`
- [x] `## Checkpoint Protocol` with `### Checkpoint File Format` and `### When to Checkpoint`
- [x] `## Determine Mode` with `### Flag Parsing` and mode list (status / empty / [CompanyName])
- [x] `## Lightweight Mode` (Gap-Reader downgraded; Counter-Spy ALWAYS Opus)
- [x] `## Spawn the Team` (Step 1/2/3, then `### The X` per agent with Name/Model/Prompt/Tasks/Phase)
- [x] `## Orchestration Flow` (`### Artifact Detection` first per scribe persona, then Phases 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5 with explicit `GATE:` markers; `### Between Phases`; `### Pipeline Completion`)
- [x] `## Critical Rules`
- [x] `## Failure Recovery`
- [x] `---` separator
- [x] universal-principles BEGIN/END SHARED block (NON-ENGINEERING — engineering-principles block intentionally OMITTED)
- [x] `---` separator
- [x] communication-protocol BEGIN/END SHARED block with `write(counter-spy, ...)` substitution and Counter-Spy target name
- [x] `---` separator
- [x] `## Teammate Spawn Prompts` (8 spawn prompts, thin format)
- [x] Counter-Spy spawn prompt contains 4 explicit `WHAT YOU CHALLENGE AT GATE [N]` blocks using exact methodology names
- [x] No `model: sonnet` / `model: opus` lines inside spawn prompt code blocks (model is on the `### The X` header line above)
- [x] Mapping disambiguation: spawn prompts use "produce your full Mapping (Source Triage Log + [core artifact] + Salience Matrix)"
- [x] Cipher disambiguation: spawn prompts use "Cipher = your raw, evidence-cited finding (URL + claim + access date + metadata) before synthesis. Not literal encryption."
- [x] Integration Dependency Map (NOT Graph) used in Stack Excavator spawn prompt and persona file
- [x] Storefront Walker spawn prompt explicitly grants cross-dimensional JTBD source access with one-line justification requirement
- [x] All 8 persona files created at `plugins/conclave/shared/personas/{slug}.md`

## Validator pre-check (expected pass/fail)

- A-series structural markers: PASS (all required H2 sections present in correct order)
- B-series shared content markers: PASS (universal-principles + communication-protocol present with markers; engineering-principles INTENTIONALLY ABSENT for non-engineering classification)
- Skeptic-name substitution: PASS (Counter-Spy / counter-spy used in communication-protocol substitution row)
- Persona file references: PASS (all 8 spawn prompts reference existing persona file paths)
- Gate marker count: PASS (4 explicit `GATE:` markers in Orchestration Flow at 1.5, 2.5, 3.5, 4.5)
- Skeptic gate methodology blocks: PASS (4 `WHAT YOU CHALLENGE AT GATE [N]` blocks with exact methodology names)

## Checkpoint Log

- [00:00] Task #3 claimed (scribe-f2a8); checkpoint frequency: every-step
- [00:05] Read scribe persona, all 4 approved upstream artifacts (architect, armorer, lorekeeper, forge-auditor), and 2 structural reference SKILL.md files (squash-bugs, research-market)
- [00:15] Read additional persona references (inquisitor, first-skeptic, cartographer, strategist, research-director, strategist--write-spec, chronicler, gtm-analyst, arbiter) for archetype calibration
- [00:25] All 8 persona files written to `plugins/conclave/shared/personas/` (cartomarshal, cartographer--competitor-research, storefront-walker, stack-excavator, gtm-analyst--competitor-research, strategist--competitor-research, chronicler--competitor-research, counter-spy)
- [00:40] SKILL.md draft assembled; structural template followed; auditor directives 1–4 honored; Scribe-specific directives (Mapping disambiguation, Cipher non-literal phrasing, Integration Dependency Map naming, JTBD cross-dimensional permission) wired
- [00:45] Self-verification checklist passes; submitting to forge-auditor-f2a8 for review
```
