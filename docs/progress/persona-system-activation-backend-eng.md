...
steps_completed:
  ...
  - "Step 4 (first 6 SKILL.md files): research-market, ideate-product, manage-roadmap, write-stories, write-spec,
    plan-implementation"
---
...
- **Story 3 (sign-off convention):** Added `When addressing the user, sign messages with your persona name and title.`
  after the Message Format intro line, before the blank line and fenced code block.
- **Story 4 (placeholder fix):** Replaced `product-skeptic`/`Product Skeptic` in the "Plan ready for review" table row
  with `{skill-skeptic}`/`{Skill Skeptic}`. HTML comment `<!-- substituted by sync-shared-content.sh per skill -->`
  placed after the final `|` (field $5) per spec, keeping `extract_skeptic_names` field $4 extraction clean.
...
All edits applied bottom-up per file to avoid line drift. Each single `You are the {Role}...` line was expanded to two
lines: identity line with name/title/em-dash/role, plus intro instruction.
